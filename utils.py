...
class Arguments(BaseModel):
    ...
    norm_layer: Optional[bool] = Field(False, description='During training, norm layer to be updated or not')
    ...
    mace_train_seperate: Optional[bool] = Field(False)
    ...
    mace_importance_sampling: Optional[bool] = Field(True)
    mace_num_train_epochs: Optional[int] = Field(1)
    use_gsam_mask: Optional[bool] = Field(True)
    ...
    clip_ti_decay: Optional[bool] = Field(True)
    fmn_lr_ti: Optional[float] = Field(0.001)
    fmn_scale_lr: Optional[bool] = Field(True)
    ...
    only_optimize_ca: Optional[bool] = Field(False)
    use_pooler: Optional[bool] = Field(True)
    center_crop: Optional[bool] = Field(False)
    ...
    doco_center_crop: Optional[bool] = Field(False)
    doco_hflip: Optional[bool] = Field(False)
    doco_noaug: Optional[bool] = Field(False) # appropriate True when style erasing according to official implemantation
    ...
    cpe_do_adv_learn: Optional[bool] = Field(True)
    ...
    # configs for RACE
    race_adv_train: Optional[bool] = Field(False)
    race_esd_path: Optional[str] = Field("", description="pretrained esd model's path")
    race_lasso: Optional[bool] = Field(False)
    ...
    ant_alpha_1: Optional[float] = Field(1.0)
    ant_alpha_2: Optional[float] = Field(0.5)
    ant_mask_path: Optional[str] = Field(None)
    ant_if_gradient: Optional[bool] = Field(True)

    # config for CoGFD
    cogfd_p1: Optional[float] = Field(-1.0)
    cogfd_p2: Optional[float] = Field(1.0)
    cogfd_start: Optional[int] = Field(990)
    cogfd_end: Optional[int] = Field(1000)
    cogfd_lr: Optional[float] = Field(5e-5)
    cogfd_num_train_epochs: Optional[int] = Field(1)
    cogfd_train_batch_size: Optional[int] = Field(20)
    cogfd_adam_beta_1: Optional[float] = Field(0.9)
    cogfd_adam_beta_2: Optional[float] = Field(0.999)
    cogfd_adam_weight_decay: Optional[float] = Field(0.01)
    cogfd_adam_epsilon: Optional[float] = Field(1.0e-08)
    cogfd_gradient_accumulation_steps: Optional[int] = Field(1)
    cogfd_scale_lr: Optional[bool] = Field(False)
    cogfd_use_8bit_adam: Optional[bool] = Field(False)
    cogfd_train_text_encoder: Optional[bool] = Field(False)
    cogfd_center_crop: Optional[bool] = Field(False)
    cogfd_only_optimize_ca: Optional[bool] = Field(False)
    cogfd_set_grads_to_none: Optional[bool] = Field(False)
    cogfd_use_pooler: Optional[bool] = Field(True)
    cogfd_max_train_steps: Optional[int] = Field(100)
    cogfd_lr_warmup_steps: Optional[int] = Field(0)
