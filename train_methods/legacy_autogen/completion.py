...
from openai.types.completion import Completion as openai_Completion
from openai.types.chat import ChatCompletion
from openai import APIError, APIConnectionError, BadRequestError, Timeout, RateLimitError, AuthenticationError

from train_methods.legacy_autogen.client import get_key
...
class Completion(openai_Completion):
    ...
    openai_completion_class = openai_Completion
    _total_cost = 0
    ...
    def _get_response(cls, config: Dict, raise_on_ratelimit_or_timeout=False, use_cache=True):
        ...
        while True:
            try:
                ...
                # transient error
                print(f"retrying in {retry_wait_time} seconds...", exc_info=1)
        ...
        **config,
        ...
        if config is not None:
            ...
            if temperature is not None and top_p is None:
                ...
                space["top_p"] = top_p
                print("temperature and top_p are not recommended to vary together.")
    ...
    def test(
        ...
        **config,
        ...
        for i, data_i in enumerate(data):
            print(f"evaluating data instance {i}")
