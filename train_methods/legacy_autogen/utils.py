...
from train_methods.legacy_autogen.completion import Completion
...
def improve_function(file_name, func_name, objective, **config):
    ...
    return Completion.extract_text(response)[0], response["cost"]
...
"""
    ...
    response = Completion.create({"objective": objective, "code": code, "followup": followup}, **params)
    return Completion.extract_text(response)[0], response["cost"]
...
def generate_assertions(definition: str, **config) -> tuple[str, float]:
    ...
    assertions = Completion.extract_text(response)[0]
...
class PassAssertionFilter:
    ...
    def pass_assertions(self, context, response, **_):
        ...
        responses = Completion.extract_text(response)
...
def implement(
    ...
    assertions: str | Callable[[str], tuple[str, float]] | None = generate_assertions,
) -> tuple[str, float]:
